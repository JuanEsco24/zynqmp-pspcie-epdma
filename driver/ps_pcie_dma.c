...
exp_dma_read (struct file *file,
    ...
    size_t transfer_size = MAX_TRANSFER_LENGTH/2;
    loff_t dma_offset; 
    bool ping = true;
	...
	while (remaining > 0) {
		...
		if (ping) {
			// Check if completion has already occurred
			// if (!try_wait_for_completion(&chan->ping_completion)) {
			// reinit_completion(&chan->ping_completion);}
			dma_offset = RX_PING_BUFFER_ADDR;
			reinit_completion(&chan->ping_completion);
			wait_for_completion(&chan->ping_completion);
		} else {
			// Check if completion has already occurred
			// if (!try_wait_for_completion(&chan->pong_completion)) {
			// reinit_completion(&chan->pong_completion);}
	       dma_offset = RX_PONG_BUFFER_ADDR;
		   reinit_completion(&chan->pong_completion);
		   wait_for_completion(&chan->pong_completion);
		}
...
exp_dma_write (struct file *file,
    ...
    ssize_t total_read = 0;
	chan =   file->private_data;
	volatile ssize_t chunk=(length/4)/2;//Number of samples
...
static int expDmaCheckInterruptStatus(expresso_dma_chan_t *chan) {
	...
	if (status & DMA_INTSTATUS_SWINTR_BIT) {
        ...
        if (scratch0_value % 2 == 0) {
			...
			//printk(KERN_INFO"Received SW Ping interrupt with SC0:%u\n\r",scratch0_value);
			//chan->ping_completion=true;
        } else {
            complete(&chan->pong_completion);
			//printk(KERN_INFO"Received SW Pong interrupt with SC0:%u\n\r",scratch0_value);
